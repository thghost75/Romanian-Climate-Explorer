...
function App(){
  ...
  const [recordArea,setRecordArea]=useState<'station'|'national'>('station');
  const nationalRecords=section==='records'&&recordArea==='national';
  ...
  const fullStart=Math.min(...(view==='compare'?compared:[station]).map(s=>Number(s.first.slice(0,4))));
  const start=period==='full'?fullStart:Number(period);
  useEffect(()=>{setYear(y=>Math.max(start,Math.min(LAST_YEAR,y)));},[start]);
  const selectStation=(id:string)=>{setCompare(ids=>ids.map(value=>value===id?selected:value));setSelected(id);setNotice('');};
  const archiveChange=(change:ArchiveChange)=>{if(change.recordArea)setRecordArea(change.recordArea);if(change.station)selectStation(change.station);if(change.tab)setSection(change.tab);if(change.year!==undefined)setArchiveYear(change.year);};
  ...
  const changeStart=(value:string)=>{setPeriod(value);};
  function exportCSV(){
  ...
  return <div className="app">
    ...
    <div className="workspace">
      ...
      <main>
        ...
        <div className="toolbar"><nav aria-label="Workspace views">{([{id:'dashboard',label:'Annual overview',icon:Grid2X2},{id:'heatmap',label:'Anomaly heatmap',icon:Activity},{id:'compare',label:'Compare stations',icon:TrendingUp}] as const).map(v=><button key={v.id} aria-pressed={view===v.id} className={view===v.id?'active':''} onClick={()=>setView(v.id)}><v.icon size={16}/>{v.label}</button>)}</nav><label className="period">Period<select aria-label="Chart period" value={period} onChange={e=>changeStart(e.target.value)}><option value="full">Full available record · {fullStart}–{LAST_YEAR}</option><option value="1961">1961–{LAST_YEAR}</option><option value="1991">1991–{LAST_YEAR}</option><option value="2011">2011–{LAST_YEAR}</option></select></label></div>
        <p className="record-coverage">{tidy(station.name)} observations: {station.first} to {station.last}. Availability varies by variable; missing or incomplete annual values remain gaps. <button className="text-button" onClick={()=>{setArchiveYear(year);setSection('history');}}>View {year} in History <ChevronRight size={14}/></button></p>
...
function TimeChart({id,stationList,metric,year,setYear,start,anomalies=false}:{id:string;stationList:Station[];metric:Metric;year:number;setYear:(y:number)=>void;start:number;anomalies?:boolean}){
  ...
  const tickStep=Math.max(5,Math.ceil((LAST_YEAR-start)/4/5)*5);
  const yearTicks=[start,...years.filter(y=>y%tickStep===0&&y-start>tickStep/2&&LAST_YEAR-y>tickStep/2),LAST_YEAR];
...
function Heatmap({station,metric,year,setYear,start,compact=false}:{station:Station;metric:Metric;year:number;setYear:(y:number)=>void;start:number;compact?:boolean}){
  const firstDecade=Math.floor(start/10)*10;
  const decades=Array.from({length:Math.floor((LAST_YEAR-firstDecade)/10)+1},(_,i)=>firstDecade+i*10);
