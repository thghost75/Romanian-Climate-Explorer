...
import { Activity, ArrowDownToLine, Check, ChevronRight, CloudRain, Grid2X2, MapPin, Search, Thermometer, TrendingUp, WifiOff, X, CalendarDays, Trophy, History, Zap, Sun, Moon } from 'lucide-react';
...
function App(){
  ...
  return <div className="app">
    <header className="topbar"><div className="brand"><span className="flag" aria-label="Romanian flag"><i/><i/><i/></span><div><strong>Romanian Climate Explorer</strong><span>WxProbs / research workspace</span></div></div><div className="top-actions"><button className="theme-toggle" type="button" aria-label={`Switch to ${theme==='dark'?'light':'dark'} mode`} title={`Switch to ${theme==='dark'?'light':'dark'} mode`} onClick={()=>setTheme(value=>value==='dark'?'light':'dark')}>{theme==='dark'?<Sun size={18}/>:<Moon size={18}/>}<span>{theme==='dark'?'Light mode':'Dark mode'}</span></button><span className="offline"><WifiOff size={14}/> ANM climate archive</span>{section==='overview'&&<button className="export-button" aria-label="Export annual CSV" onClick={exportCSV}><ArrowDownToLine size={16}/><span>Export annual CSV</span></button>}</div></header>
    <div className="workspace">
      <aside className="sidebar"><div className="sidebar-heading"><span className="eyebrow">Station network</span><span className="count">160</span></div><label className="search"><Search size={17}/><input aria-label="Find a station" placeholder="Find a station…" value={query} onChange={e=>setQuery(e.target.value)}/>{query&&<button aria-label="Clear search" onClick={()=>setQuery('')}><X size={14}/></button>}</label><div className="list-caption">{filtered.length} stations · ANM + WMO</div><div className="station-list">{filtered.map(s=><button key={s.id} onClick={()=>selectStation(s.id)} className={'station-button '+(s.id===selected?'selected':'')} aria-pressed={s.id===selected}><MapPin size={15}/><span><strong>{tidy(s.name)}</strong><small>{s.id}</small></span>{s.id===selected&&<ChevronRight size={15}/>}</button>)}{!filtered.length&&<p className="empty-list">No matching stations. Try a name or WIGOS identifier.</p>}</div><div className="sidebar-footer"><WifiOff size={16}/><p><strong>Archive updated</strong><span>Through {snapshot.snapshotDate}<br/>Daily data checks</span></p></div></aside>
      <main>
