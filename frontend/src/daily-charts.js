...
window.CLIMATE_DAILY_CHARTS = {
 render(target,data,helpers) {
  ...
  function draw(){
   ...
   let svg='<svg class="ce-daily-svg" viewBox="0 0 '+w+' '+h+'" xmlns="http://www.w3.org/2000/svg" role="img" tabindex="0" aria-label="'+E(station+' daily temperature and rainfall in '+data.year)+'"><rect width="1120" height="610" fill="#142432"/><text x="65" y="24" font-family="Arial" font-size="19" font-weight="bold" fill="#e5edf2">'+E(station+' · Daily weather in '+data.year)+'</text><text x="65" y="46" font-family="Arial" font-size="12" fill="#abc4d2">'+E(data.station.station_id+' · '+(month?new Date(2000,month-1,1).toLocaleString("en-GB",{month:"long"}):'January–December')+' · ANM historical observations')+'</text>';
   for(let v=min;v<=max;v+=10){const y=ty(v);svg+='<line x1="'+L+'" x2="'+(w-R)+'" y1="'+y+'" y2="'+y+'" stroke="#36515f"/><text x="'+(L-10)+'" y="'+(y+4)+'" text-anchor="end" font-family="Arial" font-size="12" fill="#abc4d2">'+fmt(v,0)+'</text>';}
   for(let v=0;v<=rainMax;v+=rainStep){const y=ry(v);svg+='<line x1="'+L+'" x2="'+(w-R)+'" y1="'+y+'" y2="'+y+'" stroke="#36515f"/><text x="'+(L-10)+'" y="'+(y+4)+'" text-anchor="end" font-family="Arial" font-size="12" fill="#abc4d2">'+fmt(v,0)+'</text>';}
   svg+='<text transform="translate(18,215) rotate(-90)" text-anchor="middle" font-family="Arial" font-size="13" fill="#abc4d2">Temperature (°C)</text><text transform="translate(18,490) rotate(-90)" text-anchor="middle" font-family="Arial" font-size="13" fill="#abc4d2">Rainfall (mm)</text>';
   ...
   svg+='<path d="'+path(visible.map((d,i)=>d.values.tmean_c===null?null:[x(i),ty(d.values.tmean_c)]))+'" fill="none" stroke="#8bbbe9" stroke-width="2"/>';
   for(const seg of segments(visible,d=>d.values.tmean_c!==null)){if(seg.length===1){const i=seg[0];svg+='<circle cx="'+x(i)+'" cy="'+ty(visible[i].values.tmean_c)+'" r="2.5" fill="#8bbbe9"/>';}}
   ...
   visible.forEach((d,i)=>{
    ...
    if(d.raw?.precip_trace&&v!==null)svg+='<circle cx="'+x(i)+'" cy="'+(rainBottom+5)+'" r="2" fill="#ffc18e"><title>'+E(d.date+': trace precipitation')+'</title></circle>';
    if(v===null)svg+='<rect x="'+(x(i)-barWidth/2)+'" y="'+(rainBottom+10)+'" width="'+barWidth+'" height="3" fill="#a9b2b9"/>';
    if(d.raw&&(Object.keys(d.raw.excluded_variables).length||d.raw.verification_notes.length||d.raw.quality_flags.length))svg+='<circle cx="'+x(i)+'" cy="'+(T-8)+'" r="2.5" fill="#f6b966"><title>'+E(d.date+': quality details available')+'</title></circle>';
    if(month?i%5===0:d.date.endsWith("-01"))svg+='<text x="'+x(i)+'" y="579" text-anchor="middle" font-family="Arial" font-size="12" fill="#abc4d2">'+E(month?d.date.slice(8):new Date(d.date+"T12:00:00").toLocaleString("en-GB",{month:"short"}))+'</text>';
   });
   if(!temps.length)svg+='<text x="550" y="220" text-anchor="middle" font-family="Arial" fill="#abc4d2">No eligible temperature observations</text>';
   if(!rains.length)svg+='<text x="550" y="480" text-anchor="middle" font-family="Arial" fill="#abc4d2">No eligible rainfall observations</text>';
   svg+='<text x="65" y="603" font-family="Arial" font-size="11" fill="#abc4d2">Source: ANM · Blue band: Tmin–Tmax · Blue line: mean · Dashed gold: normal · Brown dots: trace · Grey marks: missing rain · Gold dots: QC details</text><line class="ce-day-guide" x1="'+x(selected)+'" x2="'+x(selected)+'" y1="'+T+'" y2="'+rainBottom+'" stroke="#a6c6d6" stroke-dasharray="3 3" opacity=".5"/></svg>';
   main.innerHTML=svg;
  ...
  function imageSVG(){
   ...
   inlineSVGTheme(main.querySelector("svg"),s);
   ...
   for(const [key,value] of Object.entries({x:65,y:height+25,"font-family":"Arial","font-size":14,fill:getComputedStyle(document.documentElement).getPropertyValue("--ink").trim()}))copyright.setAttribute(key,value);
