"""Station top-ten rankings are daily observations, with cutoff ties and QC."""
import unittest
...
class StationRankingTests(unittest.TestCase):
    ...
    def test_month_year_leap_day_and_missing_values(self):
        ...
        self.assertEqual([r['value'] for r in month['ranking']],[-12,-11,-10,7,7])
        self.assertEqual([r['rank'] for r in month['ranking']],[1,2,3,4,4])
    ...
    def test_tenth_place_ties_and_station_isolation(self):
        ...
        self.assertEqual([r['rank'] for r in result['ranking']],list(range(1,11))+[10])
        self.assertEqual([r['value'] for r in result['ranking']],list(range(1,11))+[10])
