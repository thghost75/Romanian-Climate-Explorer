(() => {
"use strict";
const host=document.getElementById("climateExplorer"); if(!host)return;
...
async function api(endpoint,params={}){
 ...
 const response=await fetch(url),data=await response.json();if(!response.ok)throw Error(data.error||"Climate request failed");
...
async function saveChart(panel,format){
 ...
 textLines(panel.querySelector("h3").textContent,22,"#172126");
 textLines(panel.dataset.exportContext,13,"#526b76");
 for(const item of panel.querySelectorAll(".ce-legend > span")){
  const dot=item.querySelector(".ce-dot");
  if(dot){add("circle",{cx:35,cy:y-4,r:4,fill:getComputedStyle(dot).backgroundColor});add("text",{x:48,y,"font-family":"Arial","font-size":13,fill:"#526b76"},item.textContent);y+=21;}
  else textLines(item.textContent,12,"#526b76");
 }
 y+=8;
 ...
 y+=plotHeight+25;
 textLines("Source: ANM / odp.meteoromania.ro · Eligible observations only; missing values remain gaps.",12,"#526b76");
 if(panel.dataset.exportName.includes("Cumulative rainfall"))textLines("Observed rainfall is a subtotal when coverage is incomplete. Missing daily normals stop the reference accumulation.",12,"#526b76");
 y+=6;
 textLines("© WxProbs",14,"#172c3c");
 ...
 const background=document.createElementNS(ns,"rect");background.setAttribute("width","100%");background.setAttribute("height","100%");background.setAttribute("fill","#f6f9fb");svg.prepend(background);
...
function modal(title,body){
 const previous=document.activeElement,wrap=document.createElement("div");wrap.className="ce-modal";wrap.innerHTML='<section class="ce-modal-content" role="dialog" aria-modal="true" aria-label="'+E(title)+'"><button class="ce-modal-close" aria-label="Close dialog">Close ×</button><h3>'+E(title)+'</h3><div class="ce-modal-body">'+body+"</div></section>";document.body.append(wrap);
...
const tabIcons={"overview":"<rect x=\"3\" y=\"3\" width=\"7\" height=\"7\" rx=\"1\"/><rect x=\"14\" y=\"3\" width=\"7\" height=\"7\" rx=\"1\"/><rect x=\"3\" y=\"14\" width=\"7\" height=\"7\" rx=\"1\"/><rect x=\"14\" y=\"14\" width=\"7\" height=\"7\" rx=\"1\"/>","records":"<path d=\"M8 3h8v7a4 4 0 0 1-8 0V3ZM8 5H4v3a4 4 0 0 0 4 4m8-7h4v3a4 4 0 0 1-4 4M12 14v7m-4 0h8\"/>","temperature":"<path d=\"M9 14V5a3 3 0 0 1 6 0v9a5 5 0 1 1-6 0ZM12 8v9\"/><circle cx=\"12\" cy=\"18\" r=\"1\"/>","rainfall":"<path d=\"M12 3s-7 8-7 12a7 7 0 0 0 14 0c0-4-7-12-7-12ZM9 16a3 3 0 0 0 3 3\"/>","extremes":"<path d=\"m13 2-9 12h7l-1 8 10-13h-8l1-7Z\"/>","history":"<path d=\"M3 11a9 9 0 1 1 2 7M3 5v6h6m3-5v6l4 2\"/>"};
host.innerHTML='<div class="ce-heading ce-intro"><div><div class="ce-eyebrow">Romania · Historical weather archive</div><h2>A closer look at Romania’s climate.</h2><p class="ce-muted">Explore local patterns, remarkable records and over 150 years of observations.</p></div><div class="ce-view-switch" aria-label="Explorer views"><button id="ce-explorer-nav">Climate Explorer</button> <button id="ce-day-nav">On This Day</button></div></div>'+
'<div class="ce-controls"><label>Climate normal<select id="ce-normal">'+["1961-1990","1971-2000","1981-2010","1991-2020"].map(n=>'<option '+(n===state.normal?"selected":"")+'>'+n+'</option>').join("")+'</select></label><label>Calendar month<select id="ce-month">'+Array.from({length:12},(_,i)=>'<option value="'+(i+1)+'" '+(i+1===state.month?'selected':'')+'>'+new Date(2000,i,1).toLocaleString('en-GB',{month:'long'})+'</option>').join('')+'</select></label><label>Calendar day<select id="ce-calendar-day">'+Array.from({length:new Date(2000,state.month,0).getDate()},(_,i)=>'<option '+(i+1===state.day?'selected':'')+'>'+(i+1)+'</option>').join('')+'</select></label></div>'+
'<div id="ce-explorer"><section class="ce-panel ce-layout"><div class="ce-map-panel"><div class="ce-map-heading"><h3>The station network</h3><span id="ce-map-count">Loading locations…</span></div><div class="ce-controls"><label>Map layer<select id="ce-mode"><option value="stations">Stations</option><option value="temperature">Daily mean temperature normal</option><option value="rainfall">Daily precipitation normal</option><option value="anomaly">Annual temperature anomaly</option><option value="extremes">Calendar-day highest Tmax</option></select></label><label id="ce-map-year-label" hidden>Year<input id="ce-map-year" type="number" min="1800" max="2200" value="'+state.year+'"></label></div><div id="ce-map"></div><p id="ce-map-note" class="ce-muted ce-small"></p></div><div class="ce-station-picker"><h3>Find your station</h3><label>Search the archive<input id="ce-search" type="search" placeholder="Station name or WIGOS ID…" autocomplete="off"></label><p id="ce-station-count" class="ce-small ce-muted"></p><div id="ce-stations" class="ce-stations" aria-label="Station catalogue"></div><p id="ce-location-summary" class="ce-muted ce-small"></p></div></section>'+
'<div id="ce-station-title"></div><nav class="ce-tabs" aria-label="Station sections">'+["overview","records","temperature","rainfall","extremes","history"].map(t=>'<button data-tab="'+t+'"><svg class="ce-icon" viewBox="0 0 24 24" aria-hidden="true">'+tabIcons[t]+'</svg>'+t+'</button>').join("")+'</nav><div id="ce-content" aria-live="polite"></div></div><div id="ce-on-day" hidden></div>'+help+
'<footer class="ce-help">Source: Administrația Națională de Meteorologie · station archive snapshot through 17 September 2026. Historical metadata may not describe past station locations. No national trend attribution is implied.</footer>';
const $=id=>host.querySelector("#"+id);
function stationList(query=""){
 const key=x=>x.normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase();
 const rows=state.catalogue.filter(s=>key((s.station_name||"")+" "+s.station_id).includes(key(query)));
 $("ce-station-count").textContent=rows.length+" matching stations";
 const total=state.catalogue.length,missing=state.catalogue.filter(s=>!s.has_coordinates).length;
 $("ce-location-summary").textContent=missing?"All "+total+" stations are searchable. "+missing+" have no coordinates in the catalogue and are not shown on the map.":"All "+total+" stations are searchable and shown on the map.";
 $("ce-stations").innerHTML=rows.map(s=>'<button data-station="'+E(s.station_id)+'" class="'+(s.station_id===state.station?"active":"")+'"><span>'+E(s.station_name||s.station_id)+'<small style="display:block;opacity:.7">'+E(s.station_id)+'</small></span><small>'+(!s.has_coordinates?"No map location":E(fmt(s.completeness_percent))+"%")+'</small></button>').join("");
 $("ce-stations").querySelectorAll("button").forEach(b=>b.onclick=()=>selectStation(b.dataset.station));
}
async function map(){
 const request=++mapSerial;
 try{
 const d=await api("map",{...params(),mode:state.mode});if(request!==mapSerial)return;
 const projection=(lon,lat)=>[30+(lon-20.1)*62,440-(lat-43.4)*62/Math.cos(46*Math.PI/180)];
 let svg='<svg class="ce-map" viewBox="0 0 650 470" role="group" aria-label="Official climate station locations in Romania">';
 const draw=g=>{if(g.type==="FeatureCollection")g.features.forEach(draw);else if(g.type==="Feature")draw(g.geometry);else if(g.type==="GeometryCollection")g.geometries.forEach(draw);else if(g.type==="Polygon"||g.type==="MultiPolygon"){const polys=g.type==="Polygon"?[g.coordinates]:g.coordinates;polys.forEach(poly=>{svg+='<path class="ce-boundary" fill-rule="evenodd" d="'+poly.map(ring=>ring.map((c,i)=>(i?"L":"M")+projection(...c).join(",")).join("")+"Z").join("")+'"/>';});}};
 if(state.boundary)draw(state.boundary);
 const values=d.stations.map(s=>s.value).filter(v=>v!==null),lo=Math.min(...values),hi=Math.max(...values);
 const mapped=d.stations.filter(s=>s.has_coordinates);
 $("ce-map-count").textContent=mapped.length+" mapped locations";
 mapped.forEach(s=>{const [x,y]=projection(s.longitude,s.latitude),color=state.mode==="stations"?"#087b98":s.value===null?"#9aa8ae":"hsl("+(220-210*(s.value-lo)/(hi-lo||1))+" 60% 46%)";const title=(s.station_name||s.station_id)+(state.mode==="stations"?"":": "+fmt(s.value)+(state.mode==="rainfall"?" mm":" °C")+" · n="+(s.sample_count??0)+" "+s.sample_unit)+(s.needs_verification?" · Historical observation flagged for verification":"");svg+='<circle class="'+(state.mode==="stations"?"ce-station-dot ":"")+(s.station_id===state.station?"ce-selected":"")+'" tabindex="0" role="button" aria-label="'+E(title)+'" data-station="'+E(s.station_id)+'" cx="'+x+'" cy="'+y+'" r="'+(s.station_id===state.station?6:4)+'" fill="'+color+'"><title>'+E(title)+'</title></circle>';});
 $("ce-map").innerHTML=svg+"</svg>";
 $("ce-map").querySelectorAll("circle").forEach(c=>{c.onclick=()=>selectStation(c.dataset.station);c.onkeydown=e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();selectStation(c.dataset.station);}};});
 $("ce-map-note").innerHTML=(state.mode==="stations"?mapped.length+" official station locations · ANM / WMO OSCAR":'<span class="ce-colorbar"></span> '+E(values.length?fmt(lo)+" to "+fmt(hi)+(state.mode==="rainfall"?" mm":" °C"):"No eligible values")+" · "+(state.mode==="anomaly"?state.year+" relative to 1991–2020":state.mode==="extremes"?"All available years · "+String(state.month).padStart(2,"0")+"-"+String(state.day).padStart(2,"0"):E(state.normal))+" · grey = no eligible value")+" · Station markers only";
 }catch(e){if(request===mapSerial)failure($("ce-map"),e);}
}
function clampStationYear(){const s=state.catalogue.find(s=>s.station_id===state.station);if(s){state.year=Math.max(s.first_year,Math.min(s.last_year,state.year));$("ce-map-year").value=state.year;}}
async function selectStation(sid){state.station=sid;clampStationYear();stationList($("ce-search").value);map();await renderTab();}
...
function renderOverview(node,d){
 ...
 node.querySelector("#ce-open-daily").onclick=()=>{state.tab="history";renderTab();};
...
function renderRecords(node,d){
 ...
 show(d);let id=0;
 node.querySelector("#ce-record-scope").onchange=async e=>{const n=++id;node.querySelector("#ce-record-table").textContent="Loading records…";try{const data=await api("records",{...params(),scope:e.target.value});if(n===id)show(data);}catch(e){failure(node.querySelector("#ce-record-table"),e);}};
...
function renderExtremes(node,initial){
 ...
 let current=initial,kind="heatwave",page=0,request=0,filtered=initial;
 function show(){
 ...
 try{const result=await api("events",{...params(),kind:k,start:node.querySelector("#ce-start").value,end:node.querySelector("#ce-end").value,threshold:node.querySelector("#ce-threshold").value});if(id!==request)return;current=result;kind=k;page=0;show();}catch(e){if(id===request)failure(node.querySelector("#ce-event-table"),e);}
 }
...
async function renderHistory(node,d){
 ...
 node.querySelector("#ce-history-load").onclick=()=>{const y=Number(node.querySelector("#ce-history-year").value);if(!Number.isInteger(y)||y<s.first_year||y>s.last_year){modal("Choose a year","Choose a year between "+s.first_year+" and "+s.last_year+".");return;}state.year=y;renderTab();};
...
async function onDay(){
 ...
 node.querySelectorAll("[data-day-station]").forEach(b=>b.onclick=()=>{state.station=b.dataset.dayStation;location.hash="climate";route();});
...
function route(){clampStationYear();const isDay=location.hash==="#on-this-day";$("ce-explorer").hidden=isDay;$("ce-on-day").hidden=!isDay;$("ce-day-nav").classList.toggle("active",isDay);$("ce-explorer-nav").classList.toggle("active",!isDay);if(isDay)onDay();else{stationList($("ce-search").value);map();renderTab();}}
$("ce-explorer-nav").onclick=()=>{location.hash="climate";route();};$("ce-day-nav").onclick=()=>{location.hash="on-this-day";route();};
$("ce-search").oninput=e=>stationList(e.target.value);
$("ce-normal").onchange=e=>{state.normal=e.target.value;route();};
$("ce-month").onchange=e=>{state.month=Number(e.target.value);const max=new Date(2000,state.month,0).getDate();state.day=Math.min(state.day,max);$("ce-calendar-day").innerHTML=Array.from({length:max},(_,i)=>'<option '+(i+1===state.day?'selected':'')+'>'+(i+1)+'</option>').join("");route();};
$("ce-calendar-day").onchange=e=>{state.day=Number(e.target.value);route();};
$("ce-mode").onchange=e=>{state.mode=e.target.value;$("ce-map-year-label").hidden=state.mode!=="anomaly";map();};
$("ce-map-year").onchange=e=>{const y=Number(e.target.value);if(Number.isInteger(y)&&y>=1800&&y<=2200){state.year=y;map();}};
host.querySelectorAll("[data-tab]").forEach(b=>b.onclick=()=>{state.tab=b.dataset.tab;renderTab();});
window.addEventListener("hashchange",route);
(async()=>{try{const [catalogue,boundary]=await Promise.all([api("stations"),fetch("romania.geojson").then(r=>{if(!r.ok)throw Error("Romania boundary unavailable");return r.json();})]);state.catalogue=catalogue;state.boundary=boundary;route();}catch(e){failure($("ce-content"),e);}})();
})();
